...
from MaKaC.plugins.EPayment.eWay.webinterface.pages import ePayments
from MaKaC.plugins.EPayment.eWay.webinterface import urlHandlers as localUrlHandlers
from MaKaC.plugins.EPayment.eWay import epayment as ePayment
from MaKaC.plugins.EPayment.eWay import MODULE_ID


class RHEPaymentmodifEWay( RHEPaymentModifBase ):
    _requestTag = "modifEWay"
    
    def _process( self ):
        p = ePayments.WPConfModifEPaymentEWay( self, self._conf )
        return p.display()
        
class RHEPaymentmodifEWayDataModif( RHEPaymentModifBase ):
    _requestTag = "modifEWayData"
    
    def _process( self ):
        p = ePayments.WPConfModifEPaymentEWayDataModif( self, self._conf )
        return p.display()

class RHEPaymentmodifEWayPerformDataModif( RHEPaymentModifBase ):
    ...
    def _process( self ):
        ...
        self._redirect(localUrlHandlers.UHConfModifEPaymentEWay.getURL(self._conf)) 
        



class RHEPaymentconfirmEWay( RHRegistrationFormDisplayBase ):
    _requestTag = "confirm"
    ...
    def _processIfActive( self ):
        if self._registrant is not None:
            p = ePayments.WPconfirmEPaymentEWay( self,self._conf,self._registrant)
            return p.display()
            
class RHEPaymentCancelEWay( RHRegistrationFormDisplayBase ):
    _requestTag = "cancel"
    ...
    def _processIfActive( self ):
        if self._registrant is not None:
            p = ePayments.WPCancelEPaymentEWay( self,self._conf ,self._registrant)
            return p.display()
    
                
class RHEPaymentValideParamEWay( RHConferenceBaseDisplay ):
    _requestTag = "params"
